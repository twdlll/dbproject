...
<body class="main">
<p style="color:white;font-size:25px;text-align:center">显示未派发的订单</p>
<%
String sql;
...
sql="select * from orders,food_view where orders.食品号=food_view.食品号 and "+
    "是否派送='否' and 是否完成='否' and orders.商家Id='"+userId+"'";
}
else{
sql="select * from orders,food_view where orders.食品号=food_view.食品号 and "+
    "是否派送='否' and 是否完成='否' and 买家Id='"+userId+"'";}
//测试代码
//System.out.println(sql);
ResultSet rs=aboutSQL.executeQuery(sql);
while(rs.next()){
out.println("<hr>");
out.println("<form action=\"cancel_food.jsp\" Method=\"post\" target=\"_self\" style=\"margin:10px auto;\">");
...
if(flag){
out.println("<p style=\"text-align:center\"><input type=\"submit\" value=\"派送\" formaction=\"send_order.jsp\"/>");
out.println("</p>");
...
out.println("</table><p style=\"text-align:center\"><input type=\"submit\" value=\"取消订单\" formaction=\"cancel_order.jsp\"/></p>");
}
out.println("</form>");
}
%>
